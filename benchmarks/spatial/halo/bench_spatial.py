...
def get_parser():
    ...
    parser.add_argument('--enable-val-recv-tensors',  action='store_true', default=False,
                        help='Enable validation of recv tensors')
    parser.add_argument('--enable-val-conv',  action='store_true', default=False,
                        help='Enable validation of convolution')
    parser.add_argument('--enable-val-small-conv',  action='store_true', default=False,
                        help='Enable validation of sequential small validation with large')
    parser.add_argument('--enable-deterministic',  action='store_true', default=False,
                        help='Enable deterministic behaviour of cuDNN and PyTorch ')
    parser.add_argument('--enable-one-h-dim-kernel',  action='store_true', default=False,
                        help='Set dimension (height) of kernel to 1')
    parser.add_argument('--enable-one-w-dim-kernel',  action='store_true', default=False,
...
class halo_bench_pt2pt(nn.Conv2d):
	def __init__(self,local_rank, comm_size, kernel_size, num_spatial_parts, in_channels, out_channels, slice_method="square"):
		...
		super(halo_bench_pt2pt, self).__init__(	in_channels, 
											out_channels, 
	...
	def start_halo_exchange(self,halo_input):
		...
		for i in range(9):
			...
			if(self.neighbours[i]==1):
				temp_tensor = torch.zeros(shapes[0],shapes[1],self.shapes_recv[i][0],self.shapes_recv[i][1],dtype=torch.float,device=self.dev)
...
def create_input_vertical(kernel_size, halo_len, image_size, comm_size, rank):
	...
	np_x = np.asarray(list(range(0,args.batch_size * args.in_channels * image_size[0]*image_size[1])), dtype=np.float32)

	np_x.resize(args.batch_size, args.in_channels, image_size[0],image_size[1])

	pad_width = [(0,0), (0,0), (halo_len_height, halo_len_height), (halo_len_width, halo_len_width)]
	...
	pads = nn.ZeroPad2d((halo_len_width, halo_len_width, halo_len_height, halo_len_height))
...
def create_input_horizontal(kernel_size, halo_len, image_size, comm_size, rank):
	...
	np_x = np.asarray(list(range(0,args.batch_size * args.in_channels * image_size[0]*image_size[1]))
						, dtype=np.float32)
...
def test_output_square(image_size,output, expected_output, rank, size, mode = 'CONV'):
	...
	if(np.equal(output,expected_output).all()):
		...
		print(mode + " Validation failed Rank.............................................................................:"+str(rank))
...
def test_output_horizontal(image_size,output, expected_output, rank, size, mode = 'CONV'):
	#only padding ==  halo_len case is supported 
	image_height_local = int( image_size[0] / (size) )
	image_width_local = int( image_size[1] / (size) )
	expected_out_width = image_width_local
...
def test_output_recv(output, expected_output,rank):
	...
	if(np.equal(np_out,expected_output).all()):
		...
		print("Recv Tensor Rank:"+str(rank),  np_out.astype('int')[uneq],  expected_output.astype('int')[uneq])
...
if(args.slice_method == "vertical"):
	input_tensor_local, expected_output_recv = create_input_vertical(kernel_size = kernel_size,
															halo_len=halo_len, 
	...
	input_tensor_local, expected_output_recv = create_input_horizontal(kernel_size = kernel_size,
															halo_len=halo_len, 
															image_size=image_size, 
															comm_size = size, 
															rank = rank)
elif(args.slice_method == "square"):
	input_tensor_local, expected_output_recv = create_input_square(kernel_size = kernel_size,
															halo_len=halo_len, 
															image_size=image_size, 
															comm_size = size, 
															rank = rank)

print("Size of input:{} Size of Output:{}".format(input_tensor_local.shape, expected_output_recv.shape))


b_pt2pt = halo_bench_pt2pt(local_rank=rank, comm_size=size, kernel_size = kernel_size, num_spatial_parts=args.num_spatial_parts, in_channels = args.in_channels, out_channels = args.out_channels,  slice_method=args.slice_method)
...
input_seq = np.asarray(list(range(0,args.batch_size * args.in_channels * image_size[0]*image_size[1])), dtype=np.float32)
...
conv_seq = nn.Conv2d(args.in_channels, 
					 args.out_channels, 
					 kernel_size = kernel_size,
					 stride=1, 
...
if ENABLE_VAL_CONV:
	if(args.slice_method=="vertical"):
		test_output_vertical(image_size,output, expected_output, rank, size, mode ='CONV')
	elif(args.slice_method=="horizontal"):
		test_output_horizontal(image_size,output, expected_output, rank, size, mode ='CONV')
...
if ENABLE_VAL_SMALL_CONV:
	...
	conv_seq_small = nn.Conv2d(args.in_channels, 
						 args.out_channels, 
						 kernel_size = kernel_size, 
						 stride=1, 
						 padding=0, 
	...
	expected_output_recv = torch.tensor(expected_output_recv,dtype=torch.float,device=dev)
	...
	if(args.slice_method=="vertical"):
		...
		test_output_vertical(image_size,output, expected_output, rank, size, mode = 'SMALL CONV')

	elif(args.slice_method=="horizontal"):

		test_output_horizontal(image_size,output, expected_output, rank, size, mode = 'SMALL CONV')

	elif(args.slice_method=="square"):

		test_output_square(image_size,output, expected_output, rank, size, mode = 'SMALL CONV')
