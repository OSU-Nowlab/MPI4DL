...
def validate_config(num_spatial_parts, comm_size):
	...
	assert comm_size == num_spatial_parts, "Spatial parts {num_spatial_parts} is not qual to world size." #TBD: working on changes to support comm_size >= num_spatial_parts
...
class halo_bench_pt2pt(nn.Conv2d):
	def __init__(self, local_rank, comm_size, num_spatial_parts, slice_method, halo_len, in_channels, out_channels):
...
def create_input_square(halo_len, image_size, comm_size, rank):
	...
	expected_output.resize(1,1, image_size[0]+2*halo_len,image_size[1]+2*halo_len)
...
def run_benchmark(rank, size, hostname):
	...
	b_pt2pt = halo_bench_pt2pt(local_rank=rank, comm_size = size, num_spatial_parts= num_spatial_parts, slice_method = slice_method, halo_len= halo_len,  in_channels = 1, out_channels = 256)
