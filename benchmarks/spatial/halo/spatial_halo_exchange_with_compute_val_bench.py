...
class halo_bench_pt2pt(nn.Conv2d):
	def __init__(self,local_rank, num_spatial_parts, halo_len, in_channels, out_channels):
...
def test_output_recv(output, expected_output,rank):
	...
	if(np.equal(np_out,expected_output).all()):
		...
		print("Recv Rank:"+str(rank), np_out.astype('int')[uneq],  expected_output.astype('int')[uneq])
...
b_pt2pt = halo_bench_pt2pt(local_rank=rank, num_spatial_parts=num_spatial_parts, halo_len=halo_len, in_channels = args.in_channels, out_channels = args.out_channels)
