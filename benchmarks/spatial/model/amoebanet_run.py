...
def verify_config():
    ...
    if args.slice_method == "square":
        assert isPowerTwo(
            int(image_size / math.sqrt(num_spatial_parts_list[0]))
        ), "Image size of each partition should be power of Two"
    else:
        assert isPowerTwo(
            int(image_size / num_spatial_parts_list[0])
        ), "Image size of each partition should be power of Two"
...
def split_input(inputs):
    if args.slice_method == "square":
        image_height_local = int(image_size / math.sqrt(num_spatial_parts_list[0]))
        image_width_local = int(image_size / math.sqrt(num_spatial_parts_list[0]))

        total_rows = int(math.sqrt(num_spatial_parts_list[0]))
        total_cols = int(math.sqrt(num_spatial_parts_list[0]))

        # current position of rank in matrix of math.sqrt(num_spatial_parts_list[0]) * math.sqrt(num_spatial_parts)
        ...
        image_height_local = int(image_size / num_spatial_parts_list[0])
        image_width_local = int(image_size / num_spatial_parts_list[0])

        start_left = local_rank * image_width_local
        end_right = (local_rank + 1) * image_width_local

        if local_rank == num_spatial_parts_list[0] - 1:
