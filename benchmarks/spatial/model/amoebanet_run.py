...
def verify_config():
    ...
    if args.slice_method == "square":
        assert isPowerTwo(int(image_size/math.sqrt(num_spatial_parts))), "Image size of each partition should be power of Two"
    else:
        assert isPowerTwo(int(image_size/num_spatial_parts)), "Image size of each partition should be power of Two"

verify_config()
  

mpi_comm = gems_comm.MPIComm(split_size=split_size ,ENABLE_MASTER=False, ENABLE_SPATIAL=True, num_spatial_parts=num_spatial_parts, spatial_size=spatial_size, LOCAL_DP_LP=LOCAL_DP_LP)
sync_allreduce = gems_comm.SyncAllreduce(mpi_comm)


# spatial_allreduce_ranks = [i for i in range(num_spatial_parts)]
# spatial_allreduce_ranks = []
...
model_seq = amoebanet.amoebanetd(num_layers=num_layers, num_filters = num_filters, num_classes = num_classes)
print("length",len(model_seq), balance)
model_gen_seq = model_generator(model=model_seq, split_size=split_size,input_size=(int(batch_size/parts),3,image_size_seq, image_size_seq),balance=balance)
...
if(args.halo_d2):
	...
	model  = amoebanet_d2.amoebanetd_spatial( local_rank = local_rank%mpi_comm.total_spatial_processes,
	...
	model  = amoebanet.amoebanetd_spatial( local_rank = local_rank%mpi_comm.total_spatial_processes,
											spatial_size=spatial_size,
											num_spatial_parts= num_spatial_parts,
											mp_size=split_size,
											balance = balance,
...
model_gen = model_generator(model=model, split_size=split_size,input_size=(int(batch_size/parts),3,image_size,image_size),balance=balance,shape_list=resnet_shapes_list)
...
t_s = train_model_spatial(model_gen, local_rank,batch_size,epochs=1, spatial_size=spatial_size, num_spatial_parts=num_spatial_parts ,criterion=None,optimizer=None,parts=parts,ASYNC=True,GEMS_INVERSE=False, slice_method = args.slice_method,
							LOCAL_DP_LP=LOCAL_DP_LP,
							mpi_comm = mpi_comm)

x = torch.zeros((batch_size,3,int(image_size/2),int(image_size/2)),device='cuda')
...
if(APP==1):
	...
	my_dataloader = torch.utils.data.DataLoader(trainset, batch_size=times * batch_size,
	...
	my_dataloader = torch.utils.data.DataLoader(my_dataset, batch_size=batch_size*times,
											  shuffle=False, num_workers=0,pin_memory=True)
