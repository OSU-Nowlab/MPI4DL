...
mpi_comm_first = gems_comm.MPIComm(split_size=split_size ,ENABLE_MASTER=False, ENABLE_SPATIAL=True, num_spatial_parts=num_spatial_parts, spatial_size=spatial_size, LOCAL_DP_LP=LOCAL_DP_LP)
mpi_comm_second = gems_comm.MPIComm(split_size=split_size ,ENABLE_MASTER=True, ENABLE_SPATIAL=True, num_spatial_parts=num_spatial_parts, spatial_size=spatial_size, LOCAL_DP_LP=LOCAL_DP_LP, DISABLE_INIT=True)
...
model_gen_seq.ready_model(split_rank=mpi_comm_second.split_rank,GET_SHAPES_ON_CUDA=True)

image_size_times = int(image_size / image_size_seq)

resnet_shapes_list = get_shapes_spatial(shape_list = model_gen_seq.shape_list, 
...
if(args.halo_d2):
	...
	model1  = amoebanet_d2.amoebanetd_spatial( local_rank = mpi_comm_first.local_rank%mpi_comm_first.total_spatial_processes,
											spatial_size=spatial_size,
	...
	model2  = amoebanet_d2.amoebanetd_spatial( local_rank = mpi_comm_second.local_rank%mpi_comm_second.total_spatial_processes,
											spatial_size=spatial_size,
											num_spatial_parts= num_spatial_parts,
											mp_size=split_size,
											balance = balance,
	...
	model1  = amoebanet.amoebanetd_spatial( local_rank = mpi_comm_first.local_rank%mpi_comm_first.total_spatial_processes,
											spatial_size=spatial_size,
											num_spatial_parts= num_spatial_parts,
											mp_size=split_size,
											balance = balance,
	...
	model2  = amoebanet.amoebanetd_spatial( local_rank = mpi_comm_second.local_rank%mpi_comm_second.total_spatial_processes,
											spatial_size=spatial_size,
											num_spatial_parts= num_spatial_parts,
											mp_size=split_size,
											balance = balance,
...
model_gen1 = model_generator(model=model1, split_size=split_size,input_size=(int(batch_size/parts),3,image_size,image_size),balance=balance,shape_list=resnet_shapes_list)
...
model_gen2 = model_generator(model=model2, split_size=split_size,input_size=(int(batch_size/parts),3,image_size,image_size),balance=balance,shape_list=resnet_shapes_list)
...
t_s_master = train_spatial_model_master(model_gen1, 
										model_gen2, 
										batch_size,
										spatial_size,
										num_spatial_parts,
...
if(APP==1):
	...
	my_dataset=torchvision.datasets.FakeData(size=10*batch_size*args.times, image_size=(3, image_size, image_size), num_classes=num_classes, transform=transform, target_transform=None, random_offset=0)
	my_dataloader = torch.utils.data.DataLoader(my_dataset, batch_size=batch_size*args.times,
											  shuffle=False, num_workers=0,pin_memory=True)
	...
	for i_e in range(epoch):
		...
		for i, data in enumerate(my_dataloader, 0):
			...
			if(ENABLE_MASTER_OPT):
				...
				temp_loss,temp_correct = t_s_master.run_step_allreduce(x, labels, i%2 == 1)
			else:
			...
			if(ENABLE_MASTER_OPT==False):
				sync_comm.apply_allreduce_master_master(model_gen1,model_gen2, mpi_comm_first, mpi_comm_second)
			...
			if(mpi_comm_second.local_rank==0):
				None
				print("images per sec:",batch_size/t, "Time:",t, " Time Allreduce:",t_allreduce)
