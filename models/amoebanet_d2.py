...
def avg_pool_3x3(ENABLE_SPATIAL: bool, channels: int, stride: int) -> Operation:
    if ENABLE_SPATIAL:
        module = Pool(local_rank=global_info['local_rank'],
...
def conv_1x7_7x1_d2(ENABLE_SPATIAL: bool, channels: int, stride: int) -> Operation:
    ...
    module = nn.Sequential(
        ...
        nn.ReLU(inplace=False),
        nn.Conv2d(c//4, c//4, kernel_size=(1, 7), stride=(1, stride), padding=(0, 0), bias=False),
        nn.BatchNorm2d(c//4),

        nn.ReLU(inplace=False),
        nn.Conv2d(c//4, c//4, kernel_size=(7, 1), stride=(stride, 1), padding=(0, 0), bias=False),
        nn.BatchNorm2d(c//4),
...
def conv_1x7_7x1(ENABLE_SPATIAL: bool, channels: int, stride: int) -> Operation:
    ...
    if ENABLE_SPATIAL:
        module = nn.Sequential(
            nn.ReLU(inplace=False),
            conv_spatial(local_rank=global_info['local_rank'], 
                            spatial_size=global_info['spatial_size'], 
                            num_spatial_parts=global_info['num_spatial_parts'],
                            in_channels=c, 
                            out_channels=c//4, 
...
def relu_conv_bn(ENABLE_SPATIAL:bool,
                 in_channels: int,
                 out_channels: int,
                 kernel_size: int = 1,
                 stride: int = 1,
...
class Cell_D2(nn.Module):
    def __init__(self,
        ...
        self.reduce1 = relu_conv_bn(ENABLE_SPATIAL=ENABLE_SPATIAL, in_channels=channels_prev, out_channels=channels)

        self.reduce2: nn.Module = nn.Identity()
        if reduction_prev:
            self.reduce2 = FactorizedReduce(ENABLE_SPATIAL, channels_prev_prev, channels)
        elif channels_prev_prev != channels:
            self.reduce2 = relu_conv_bn(ENABLE_SPATIAL=ENABLE_SPATIAL, in_channels=channels_prev_prev, out_channels=channels)
        ...
        if reduction:
            ...
            self.s3_layer = halo_exchange_layer(local_rank=global_info['local_rank'],
                                    spatial_size=global_info['spatial_size'], 
                                    num_spatial_parts=global_info['num_spatial_parts'],
                                    halo_len = 3, 
                                    slice_method =  global_info['slice_method'])

            self.s4_layer = halo_exchange_layer(local_rank=global_info['local_rank'],
                                        spatial_size=global_info['spatial_size'], 
                                        num_spatial_parts=global_info['num_spatial_parts'],
                                        halo_len = 2, 
                                        slice_method =  global_info['slice_method'])
...
def amoebanetd(num_classes: int = 10,
    ...
    def make_cells(ENABLE_SPATIAL: bool, reduction: bool, channels_scale: int, repeat: int) -> Iterator[Cell]:
    ...
    def reduction_cell(ENABLE_SPATIAL) -> Cell:
        return next(make_cells(ENABLE_SPATIAL, reduction=True, channels_scale=2, repeat=1))

    def normal_cells(ENABLE_SPATIAL) -> Iterator[Tuple[int, Cell]]:
        return enumerate(make_cells(ENABLE_SPATIAL, reduction=False, channels_scale=1, repeat=repeat_normal_cells))
    ...
    # AmoebaNet cells
    layers.update((f'cell1_normal{i+1}', cell) for i, cell in normal_cells(ENABLE_SPATIAL))
    layers['cell2_reduction'] = reduction_cell(ENABLE_SPATIAL)


    print('Layers:', len(layers))

    layers.update((f'cell3_normal{i+1}', cell) for i, cell in normal_cells(ENABLE_SPATIAL))
    layers['cell4_reduction'] = reduction_cell(ENABLE_SPATIAL)
    layers.update((f'cell5_normal{i+1}', cell) for i, cell in normal_cells(ENABLE_SPATIAL))
...
def amoebanetd_spatial(
    ...
    _, end_layer = get_start_end_layer_index(predicted_num_layers,balance,mp_size,local_rank=0)
