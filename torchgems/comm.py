...
class MPIComm:
	def __init__(self, split_size ,
		...
		if(ENABLE_SPATIAL==False):
			...
			self.mp_size = split_size + np.sum(num_spatial_parts) -spatial_size + (split_size - spatial_size)*(LOCAL_DP_LP-1)
		...
		if(ENABLE_SPATIAL==True and (num_spatial_parts==None or spatial_size==None)):
			assert False, ("Spatial enabled but num_spatial_parts or spatial_size is None")

		if(ENABLE_SPATIAL ==True):
			if (isinstance(num_spatial_parts, list)):
				assert spatial_size == len(num_spatial_parts),"spatial size should be equal to elements in num_spatial_parts"
		...
		if(ENABLE_SPATIAL == True):
			if(self.local_rank < self.total_spatial_processes):
				...
				self.split_rank = self.get_split_rank(num_spatial_parts, self.local_rank)
			else:
		...
		if(LOCAL_DP_LP > 1):
			self.LP_SP_Groups, self.SP_LP_group =self.create_scatter_gather_spatial_MP_comm()
	...
	def create_allreduce_comm_basic(self):
		...
		ranks = [ (self.mp_size * i) + self.local_rank for i in range(int(self.size/self.mp_size))]
	...
	def create_allreduce_comm_master(self):
		...
		if(self.ENABLE_SPATIAL):
			...
			ranks = [ temp_rank  for temp_rank in range(self.size) if(temp_rank%self.mp_size== self.local_rank or temp_rank%self.mp_size == second_rank)] 
			allreduce_grp = torch.distributed.new_group(ranks=ranks)
		return allreduce_grp
	...
	def create_allreduce_comm_spatial(self):
		...
		for j in range(self.spatial_size):
			...
			if(self.spatial_size==1):
				ranks = [ (self.num_spatial_parts * j) + i for i in range(self.num_spatial_parts) ]
			else:
				ranks = [ (sum(self.num_spatial_parts_list[:j])) + i for i in range(self.num_spatial_parts_list[j]) ]
			print(ranks)
			...
			if(self.ENABLE_MASTER):
				...
				elif(self.spatial_size==1 and second_local_rank < self.num_spatial_parts):
					self.second_spatial_allreduce_grp = temp_spatial_allreduce_grp

				elif(self.spatial_size>1):

					if(first_local_rank < np.sum(self.num_spatial_parts[:j+1]) and
							first_local_rank >= np.sum(self.num_spatial_parts[:j]) ):
						self.first_spatial_allreduce_grp = temp_spatial_allreduce_grp

					elif(second_local_rank < np.sum(self.num_spatial_parts[:j+1]) and
							second_local_rank >= np.sum(self.num_spatial_parts[:j])):
						self.second_spatial_allreduce_grp = temp_spatial_allreduce_grp


			if(self.spatial_size==1):
				spatial_allreduce_grp = temp_spatial_allreduce_grp
			elif(self.local_rank < np.sum(self.num_spatial_parts[:j+1]) and
					self.local_rank >= np.sum(self.num_spatial_parts[:j]) ):
				spatial_allreduce_grp = temp_spatial_allreduce_grp
...
class SyncAllreduce:
	...
	def sync_broadcast(self,model,src,grp_comm):
		...
		for param in model.parameters():
			torch.distributed.broadcast(param.data, src=src, group=grp_comm, async_op=False)

	def sync_model_spatial(self,model_gen):
		if(self.local_rank < self.spatial_size * self.num_spatial_parts):

			self.sync_broadcast(model_gen.models,src=math.floor(self.local_rank/self.num_spatial_parts),grp_comm=self.spatial_allreduce_grp)

	def sync_model(self,model_gen1,model_gen2):
		if(self.local_rank>=self.mp_size/2):
			
			self.sync_broadcast(model_gen1.models,src=self.local_rank, grp_comm=self.allreduce_grp)
			self.sync_broadcast(model_gen2.models,src=self.local_rank, grp_comm=self.allreduce_grp)
		else:
			
			self.sync_broadcast(model_gen2.models,src=self.mp_size - self.local_rank - 1, grp_comm=self.allreduce_grp)
			self.sync_broadcast(model_gen1.models,src=self.mp_size - self.local_rank - 1, grp_comm=self.allreduce_grp)
	...
	def get_grad_flatten(self, model,back=False):
		...
		if(back==False):
			if(self.flag_grad_call_once1 == False):
				self.grad_shape_list1, self.grad_num_element_list1 = self.get_grad_info(model)
				self.flag_grad_call_once1 = True
		else:
			if(self.flag_grad_call_once2 == False):
				self.grad_shape_list2, self.grad_num_element_list2 = self.get_grad_info(model)
		...
		for param in model.parameters():
			if(param.grad is not None):
				if flat_grad is None:
					...
					flat_grad = torch.cat((flat_grad,param.grad.data.clone().detach().view(-1)))
	...
	def modify_grads(self, model,flat_grad,grad_num_element_list, grad_shape_list):
		...
		for param in model.parameters():
			if(param.grad is not None):
				...
				param.grad.data = flat_grad[temp_count_elements:temp_count_elements+grad_num_element_list[temp_count_index]].view(grad_shape_list[temp_count_index]).clone().detach() / self.divide_bs
	...
	def apply_allreduce_master(self, model_gen1,model_gen2):
		...
		self.modify_grads(models1,flat_grad1,self.grad_num_element_list1, self.grad_shape_list1)
		self.modify_grads(models2,flat_grad2,self.grad_num_element_list2, self.grad_shape_list2)
	...
	def apply_allreduce_master_master(self, model_gen1,model_gen2, comm1, comm2):
		...
		if(comm1.split_rank <= comm2.split_rank):
			...
			dist.all_reduce(flat_grad1, op=dist.reduce_op.SUM, group= comm1.allreduce_grp_master)
			dist.all_reduce(flat_grad2, op=dist.reduce_op.SUM, group= comm2.allreduce_grp_master)
		else:
