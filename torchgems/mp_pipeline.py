...
class model_generator:
	def __init__(self, model, split_size,input_size,balance=None,shape_list = None):
		...
		if balance is not None:
			assert len(balance) == split_size, "Length of balance should be equal to split size "
	...
	def get_start_end_layer_index(self,split_rank):
		...
		if(self.balance==None):
			...
			assert (sum(self.balance)==len(self.model)), "balance and number of layers differs"
	...
	def DDP_model(self, mpi_comm, num_spatial_parts, spatial_size, bucket_size=25, local_rank = None):
		...
		if(local_rank < mpi_comm.total_spatial_processes):
			...
			self.models = DDP(self.models,  device_ids=[0],
...
class train_model:
	def __init__(self, model_gen, local_rank,batch_size,epochs,criterion=None,optimizer=None,parts=1,ASYNC=True,GEMS_INVERSE=False):
		...
		try: self.num_spatial_parts
		except AttributeError: self.num_spatial_parts=1

		try: self.split_rank
		except AttributeError: self.split_rank=local_rank

		try: self.mp_size
		except AttributeError: self.mp_size=model_gen.split_size

		try: self.split_size
		except AttributeError: self.split_size=self.mp_size
	...
	def receive_input_sync(self, part_number):
		...
		if(self.MULTIPLE_INPUT):
			for i in range(len(self.shape_list[self.split_rank-1])):
				...
				dist.recv(tensor=self.input_x_list[part_number][i], src=self.to_recv_forward,tag=tag_forward)
				tag_forward+=1

		else:
			dist.recv(tensor= self.input_x_list[part_number], src = self.to_recv_forward, tag=tag_forward)
	...
	def receive_input_async(self,part_number):
		...
		if(self.MULTIPLE_INPUT):
			...
			for i in range(len(self.shape_list[self.split_rank-1])):
				...
				req_temp = dist.irecv(tensor=self.input_x_list[part_number][i], src=self.to_recv_forward,tag=tag_forward)
	...
	def send_input_async(self,y):
		...
		if (self.MULTIPLE_OUTPUT):
			...
			for one_output in y:
				...
				req = dist.isend(tensor=one_output, dst=self.to_send_forward,tag = tag_forward)
				tag_forward+=1
				reqs.append(req)
	...
	def receive_grad_sync(self):
		...
		if(self.MULTIPLE_OUTPUT):
			for i in range(len(self.shape_list[self.split_rank])):
				...
				dist.recv(tensor=self.grad_overhead[i], src=self.to_recv_backward,tag=tag_forward)
				tag_forward+=1

		else:
			dist.recv(tensor= self.grad_overhead, src = self.to_recv_backward, tag=tag_forward)
	...
	def receive_grad_async(self):
		...
		if(self.MULTIPLE_OUTPUT):
			...
			for i in range(len(self.shape_list[self.split_rank])):
				...
				req_temp = dist.irecv(tensor=self.grad_overhead[i], src=self.to_recv_backward,tag=tag_forward)
				reqs.append(req_temp)
				tag_forward+=1
	...
	def send_grad_sync(self,input_x):
		...
		if (self.MULTIPLE_INPUT):
			...
			for i in len(input_x):
				dist.send(tensor=input_x[i].grad, dst=self.to_send_backward,tag = tag_forward)
				tag_forward+=1
		else:
	...
	def send_grad_async(self,input_x):
		...
		if (self.MULTIPLE_INPUT):
			...
			for i in range(len(input_x)):
				req = dist.isend(tensor=input_x[i].grad, dst=self.to_send_backward, tag = tag_forward)
				tag_forward+=1
				reqs.append(req)
	...
	def run_step(self,data_x,data_y):
		...
		for i in range(self.parts):
			...
			temp_y,temp_correct = self.forward_pass(data_x[start:end],data_y[start:end],part_number=i)
