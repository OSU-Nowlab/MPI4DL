	...
	def start_halo_exchange(self,halo_input):
		...
		for i  in range(9):
			if(self.neighbours[i] == 1):
				...
				to_send = halo_input[:,:, self.locations_send[i][0][0]:self.locations_send[i][0][1], self.locations_send[i][1][0]:self.locations_send[i][1][1]].clone().detach()
				#torch.cuda.synchronize()
				
				t=time.time()
				temp_req = dist.isend(to_send, self.rank_neighbours[i], tag=self.send_tag[i])
		...
		for i in range(9):
			...
			if(self.neighbours[i]==1):
				...
				temp_req = dist.irecv(tensor = self.recv_tensors[i], src=self.rank_neighbours[i] , tag=self.recv_tag[i])
				req.append(temp_req)
				self.recv_tag[i] +=1
