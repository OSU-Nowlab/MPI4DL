...
class conv_spatial(nn.Conv2d):
	def __init__(self,local_rank,spatial_size, num_spatial_parts, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros'):
		...
		assert self.halo_len == padding , "Spatial not supported yet for this configuration"
		self.halo_len = padding
		super(conv_spatial, self).__init__(in_channels, out_channels, kernel_size, stride=stride, padding=0, dilation=1, groups=1, bias=True, padding_mode='zeros')
	...
	def start_halo_exchange(self,halo_input):
		...
		for i  in range(9):
			if(self.neighbours[i] == 1):
				...
				temp_req = dist.isend(halo_input[:,:, self.locations_send[i][0][0]:self.locations_send[i][0][1], self.locations_send[i][1][0]:self.locations_send[i][1][1]].clone().detach(), self.rank_neighbours[i], tag=self.send_tag[i])
				req.append(temp_req)
				self.send_tag[i] +=1
	...
	def make_tensor_halo_compute(self,halo_input):
		...
		if(self.neighbours[1]!=1 and self.neighbours[5]==1):
			...
			vertical_tensor_right = torch.cat((padding_vertical_top_right.data.fill_(0.0000000000000000), vertical_tensor_right),axis=2)

		if(self.neighbours[1]!=1 and self.neighbours[3]==1):
			padding_vertical_top_left = halo_input[:,:,0:self.halo_len,:3*self.halo_len]
			vertical_tensor_left = torch.cat((padding_vertical_top_left.data.fill_(0.0000000000000000), vertical_tensor_left),axis=2)

		if(self.neighbours[7]!=1 and self.neighbours[5]==1):
			padding_vertical_down_right =  halo_input[:,:, -self.halo_len:, -3*self.halo_len:]
			vertical_tensor_right = torch.cat(( vertical_tensor_right, padding_vertical_down_right.data.fill_(0.0000000000000000)),axis=2)

		if(self.neighbours[7]!=1 and self.neighbours[3]==1):
			padding_vertical_down_left = halo_input[:,:,-self.halo_len:,:3*self.halo_len]
			vertical_tensor_left = torch.cat(( vertical_tensor_left, padding_vertical_down_left.data.fill_(0.0000000000000000)),axis=2)


		if(self.neighbours[3]!=1 and self.neighbours[1]==1):
			padding_horizontal_up_left = halo_input[:,:, :3*self.halo_len, :self.halo_len]
			horizontal_tensor_up = torch.cat((padding_horizontal_up_left.data.fill_(0.0000000000000000),horizontal_tensor_up),axis=3)

		if(self.neighbours[3]!=1 and self.neighbours[7]==1):
			padding_horizontal_down_left = halo_input[:,:, -3 *self.halo_len:, :self.halo_len]
			horizontal_tensor_down = torch.cat((padding_horizontal_down_left.data.fill_(0.0000000000000000),horizontal_tensor_down),axis=3)

		if(self.neighbours[5]!=1 and self.neighbours[1]==1):
			padding_horizontal_up_right = halo_input[:,:, :3*self.halo_len, -self.halo_len:]
			horizontal_tensor_up = torch.cat((horizontal_tensor_up, padding_horizontal_up_right.data.fill_(0.0000000000000000)),axis=3)

		if(self.neighbours[5]!=1 and self.neighbours[7]==1):
			padding_horizontal_down_right = halo_input[:,:, -3*self.halo_len:, -self.halo_len:]
			horizontal_tensor_down = torch.cat((horizontal_tensor_down, padding_horizontal_down_right.data.fill_(0.0000000000000000)),axis=3)
