...
class conv_spatial(nn.Conv2d):
	def __init__(self,
		...
		if (isinstance(num_spatial_parts, list)):
			...
			self.spatial_local_rank, self.num_spatial_parts = self.get_local_spatial_rank(	num_spatial_parts, 
																	local_rank)
		else:
			self.local_rank = local_rank
		...
		if halo_len is not None:
			...
			assert halo_len == 0, "Error: Custom Halo Len is not supported (only halo_len=0 is supported)"
			...
			if kernel_size[0] > 1:
				...
				if self.spatial_local_rank == 0:
					padding_left, padding_right, padding_top, padding_bottom = padding[1], self.halo_len_width, padding[0], self.halo_len_height
				elif self.spatial_local_rank == 1:
					padding_left, padding_right, padding_top, padding_bottom = self.halo_len_width, padding, padding[0], self.halo_len_height
				elif self.spatial_local_rank == 2:
					padding_left, padding_right, padding_top, padding_bottom = padding[1], self.halo_len_width,  self.halo_len_height, padding[0]
				elif self.spatial_local_rank == 3:
					padding_left, padding_right, padding_top, padding_bottom = self.halo_len_width, padding[1], self.halo_len_height, padding[0]
			else:
				padding_left, padding_right, padding_top, padding_bottom = padding[1], padding[1], padding[0], padding[0]
			...
			assert self.halo_len_height == padding[0] or self.halo_len_width == padding[1], "Spatial not supported yet for this configuration"
			# self.halo_len = padding
			padding_left, padding_right, padding_top, padding_bottom = self.halo_len_width, self.halo_len_width, self.halo_len_height, self.halo_len_height


		super(conv_spatial, self).__init__(in_channels, out_channels, kernel_size, stride=stride, padding=0, dilation=1, groups=1, bias=bias, padding_mode='zeros')

		self.pads = nn.ZeroPad2d((padding_left, padding_right, padding_top, padding_bottom))
	...
	def get_local_spatial_rank(	self, 
	...
	def get_index_locations(self):
		locations_recv = []
		locations_recv.append([[None, self.halo_len_height],
								[None,self.halo_len_width]]) 														#1

		locations_recv.append([[None, self.halo_len_height],
								[self.halo_len_width,-self.halo_len_width if self.halo_len_width else None]]) 		#2

		locations_recv.append([[None, self.halo_len_height],
								[-self.halo_len_width,None]]) 														#3

		locations_recv.append([[self.halo_len_height, -self.halo_len_height if self.halo_len_height else None],
								[None,self.halo_len_width]])														#4

		locations_recv.append([[None, None],
								[None,None]])																		#5

		locations_recv.append([[self.halo_len_height, -self.halo_len_height if self.halo_len_height else None],
								[-self.halo_len_width,None]])														#6

		locations_recv.append([[-self.halo_len_height,None],
								[None,self.halo_len_width]])														#7

		locations_recv.append([[-self.halo_len_height,None],
								[self.halo_len_width,-self.halo_len_width if self.halo_len_width else None]])		#8

		locations_recv.append([[-self.halo_len_height,None],
								[-self.halo_len_width,None]])														#9

		self.locations_recv = locations_recv

		locations_send = []
		locations_send.append([[self.halo_len_height, 2*self.halo_len_height],
								[self.halo_len_width,2 * self.halo_len_width]]) 												#1

		locations_send.append([[self.halo_len_height, 2*self.halo_len_height],
								[self.halo_len_width,-self.halo_len_width if self.halo_len_width else None]]) 					#2

		locations_send.append([[self.halo_len_height, 2*self.halo_len_height],
								[-2*self.halo_len_width,-1*self.halo_len_width]]) 												#3

		locations_send.append([[self.halo_len_height, -self.halo_len_height if self.halo_len_height else None],
								[self.halo_len_width,2 * self.halo_len_width]])													#4

		locations_send.append([[None, None],
								[None,None]])																					#5

		locations_send.append([[self.halo_len_height,-self.halo_len_height if self.halo_len_height else None],
								[-2*self.halo_len_width,-1*self.halo_len_width]])												#6

		locations_send.append([[-2*self.halo_len_height,-1*self.halo_len_height],
								[self.halo_len_width,2 * self.halo_len_width]])													#7

		locations_send.append([[-2*self.halo_len_height,-1*self.halo_len_height],
								[self.halo_len_width,-self.halo_len_width if self.halo_len_width else None]])					#8

		locations_send.append([[-2*self.halo_len_height,-1*self.halo_len_height],
								[-2*self.halo_len_width,-1*self.halo_len_width]])												#9
	...
	def get_shapes_recv(self,shapes):
		...
		shapes_recv.append([self.halo_len_height, shapes[3] - 2* self.halo_len_width])			#2
		shapes_recv.append([self.halo_len_height,self.halo_len_width])							#3

		shapes_recv.append([shapes[2] - 2* self.halo_len_height,  self.halo_len_width])			#4
		shapes_recv.append([None,None])												#5
		shapes_recv.append([shapes[2] - 2* self.halo_len_height,  self.halo_len_width])			#6

		shapes_recv.append([self.halo_len_height,  self.halo_len_width])							#7
		shapes_recv.append([self.halo_len_height,  shapes[3] - 2* self.halo_len_width])			#8
	...
	def make_tensor_halo_compute(self,halo_input):
		...
		if(self.neighbours[0]==1):
			...
			horizontal_tensor_up = torch.cat((self.recv_tensors[0],self.recv_tensors[3][:,:,:2*self.halo_len,:]),axis=2)

			horizontal_tensor_temp = torch.cat( (self.recv_tensors[1], halo_input[:,:,self.halo_len:3*self.halo_len, self.halo_len: -self.halo_len ]),axis=2)

			horizontal_tensor_up = torch.cat((horizontal_tensor_up,horizontal_tensor_temp),axis=3)
		elif(self.neighbours[1]==1):
			#concat 1 with 4 position 
			horizontal_tensor_up = torch.cat( (self.recv_tensors[1], halo_input[:,:,self.halo_len:3*self.halo_len, self.halo_len: -self.halo_len ]),axis=2)

		if(self.neighbours[2]==1):
			horizontal_tensor_temp = torch.cat( (self.recv_tensors[2], self.recv_tensors[5][:,:,:2*self.halo_len,:]),axis = 2)
		...
		if(self.neighbours[6]==1):
			...
			horizontal_tensor_down = torch.cat((self.recv_tensors[3][:,:,-2*self.halo_len:,:],self.recv_tensors[6]),axis=2)

			horizontal_tensor_temp = torch.cat( (halo_input[:,:,-3*self.halo_len:-self.halo_len, self.halo_len: -self.halo_len ],self.recv_tensors[7]),axis=2)

			horizontal_tensor_down = torch.cat((horizontal_tensor_down,horizontal_tensor_temp),axis=3)
		elif(self.neighbours[7]==1):
			#concat 7 with 4 position 
			horizontal_tensor_down = torch.cat( (halo_input[:,:,-3*self.halo_len:-self.halo_len, self.halo_len: -self.halo_len ],self.recv_tensors[7]),axis=2)

		if(self.neighbours[8]==1):
			horizontal_tensor_temp = torch.cat( ( self.recv_tensors[5][:,:,-2*self.halo_len:,:],self.recv_tensors[8]),axis = 2)
		...
		if(self.neighbours[0]==1):
			vertical_tensor_left = torch.cat((self.recv_tensors[0], self.recv_tensors[1][:,:,:,:2*self.halo_len]),axis=3)

			vertical_tensor_temp = torch.cat((self.recv_tensors[3], halo_input[:,:,self.halo_len: -self.halo_len, self.halo_len: 3*self.halo_len]),axis = 3)

			vertical_tensor_left = torch.cat((vertical_tensor_left,vertical_tensor_temp),axis=2)
		elif(self.neighbours[3]==1):
			vertical_tensor_left = torch.cat((self.recv_tensors[3], halo_input[:,:,self.halo_len: -self.halo_len, self.halo_len: 3*self.halo_len]),axis = 3)

		if(self.neighbours[6]==1):
			vertical_tensor_temp = torch.cat((self.recv_tensors[6],self.recv_tensors[7][:,:,:,:2*self.halo_len]),axis=3)
			vertical_tensor_left = torch.cat((vertical_tensor_left,vertical_tensor_temp),axis=2)
		...
		if(self.neighbours[2]==1):
			vertical_tensor_right = torch.cat(( self.recv_tensors[1][:,:,:,-2*self.halo_len:], self.recv_tensors[2]),axis=3)

			vertical_tensor_temp = torch.cat((halo_input[:,:,self.halo_len: -self.halo_len,  -3*self.halo_len:-self.halo_len], self.recv_tensors[5]),axis = 3)

			vertical_tensor_right = torch.cat((vertical_tensor_right,vertical_tensor_temp),axis=2)
		elif(self.neighbours[5]==1):
			vertical_tensor_right = torch.cat((halo_input[:,:,self.halo_len: -self.halo_len,  -3*self.halo_len:-self.halo_len], self.recv_tensors[5]),axis = 3)

		if(self.neighbours[8]==1):
			vertical_tensor_temp = torch.cat((self.recv_tensors[7][:,:,:,-2*self.halo_len:], self.recv_tensors[8]),axis=3)
			vertical_tensor_right = torch.cat((vertical_tensor_right,vertical_tensor_temp),axis=2)



		if(self.neighbours[1]!=1 and self.neighbours[5]==1):
			padding_vertical_top_right = halo_input[:,:,0:self.halo_len,-3*self.halo_len:]
			vertical_tensor_right = torch.cat((padding_vertical_top_right.data, vertical_tensor_right),axis=2)

		if(self.neighbours[1]!=1 and self.neighbours[3]==1):
			padding_vertical_top_left = halo_input[:,:,0:self.halo_len,:3*self.halo_len]
			vertical_tensor_left = torch.cat((padding_vertical_top_left.data, vertical_tensor_left),axis=2)

		if(self.neighbours[7]!=1 and self.neighbours[5]==1):
			padding_vertical_down_right =  halo_input[:,:, -self.halo_len:, -3*self.halo_len:]
			vertical_tensor_right = torch.cat(( vertical_tensor_right, padding_vertical_down_right),axis=2)

		if(self.neighbours[7]!=1 and self.neighbours[3]==1):
			padding_vertical_down_left = halo_input[:,:,-self.halo_len:,:3*self.halo_len]
			vertical_tensor_left = torch.cat(( vertical_tensor_left, padding_vertical_down_left),axis=2)


		if(self.neighbours[3]!=1 and self.neighbours[1]==1):
			padding_horizontal_up_left = halo_input[:,:, :3*self.halo_len, :self.halo_len]
			horizontal_tensor_up = torch.cat((padding_horizontal_up_left,horizontal_tensor_up),axis=3)

		if(self.neighbours[3]!=1 and self.neighbours[7]==1):
			padding_horizontal_down_left = halo_input[:,:, -3 *self.halo_len:, :self.halo_len]
			horizontal_tensor_down = torch.cat((padding_horizontal_down_left,horizontal_tensor_down),axis=3)

		if(self.neighbours[5]!=1 and self.neighbours[1]==1):
			padding_horizontal_up_right = halo_input[:,:, :3*self.halo_len, -self.halo_len:]
			horizontal_tensor_up = torch.cat((horizontal_tensor_up, padding_horizontal_up_right),axis=3)

		if(self.neighbours[5]!=1 and self.neighbours[7]==1):
			padding_horizontal_down_right = halo_input[:,:, -3*self.halo_len:, -self.halo_len:]
			horizontal_tensor_down = torch.cat((horizontal_tensor_down, padding_horizontal_down_right),axis=3)
		...
		if(horizontal_tensor_up == None and horizontal_tensor_down == None):
			...
			horizontal_tensor = torch.cat((horizontal_tensor_up,horizontal_tensor_down),axis=2)
		...
		if(vertical_tensor_left == None and vertical_tensor_right == None):
			...
			vertical_tensor = torch.cat((vertical_tensor_left,vertical_tensor_right),axis=3)
	...
	def merge_final_image(self,res_final, res_horizontal, res_vertical):
		...
		if(self.neighbours[1]==1):
			res_final = torch.cat((res_horizontal[:,:,:self.halo_len,start:end],res_final),axis=2)

		if(self.neighbours[7]==1):
			if(self.neighbours[1]==1):
				res_final = torch.cat((res_final,res_horizontal[:,:,self.halo_len:,start:end]),axis=2)
			else:
				res_final = torch.cat((res_final,res_horizontal[:,:,:,start:end]),axis=2)

		if(self.neighbours[3]==1):
			res_final =torch.cat((res_vertical[:,:,:self.halo_len,:],res_final),axis=3)

		if(self.neighbours[5]==1):
			if(self.neighbours[3]==1):
				res_final =torch.cat((res_final,res_vertical[:,:,halo_len:,:]),axis=3)
			else:
	...
	def copy_final_image(self,res_final, res_horizontal, res_vertical):
		...
		if(self.neighbours[1]==1):
			res_final[:,:,:self.halo_len,:] = res_horizontal[:,:,:self.halo_len,:]
	...
	def start_halo_exchange_nochange(self,halo_input):
		...
		for i  in range(9):
			if(self.neighbours[i] == 1):
				temp_req = dist.isend(halo_input[:,:, self.locations_send[i][0][0]:self.locations_send[i][0][1], self.locations_send[i][1][0]:self.locations_send[i][1][1]], self.rank_neighbours[i], tag=self.send_tag[i])
				req.append(temp_req)
				self.send_tag[i] +=1

		for i in range(9):
			if(self.neighbours[i]==1):
				temp_req = dist.irecv(tensor = halo_input[:,:, self.locations_recv[i][0][0]:self.locations_recv[i][0][1], self.locations_recv[i][1][0]:self.locations_recv[i][1][1]], src=self.rank_neighbours[i] , tag=self.recv_tag[i])
				req.append(temp_req)
				self.recv_tag[i] +=1
...
class halo_exchange_layer(nn.Module):
	def __init__(self,
		...
		if self.spatial_local_rank == 0:
			padding_left, padding_right, padding_top, padding_bottom = padding, halo_len, padding, halo_len
		elif self.spatial_local_rank == 1:
			padding_left, padding_right, padding_top, padding_bottom = halo_len, padding, padding, halo_len
		elif self.spatial_local_rank == 2:
			padding_left, padding_right, padding_top, padding_bottom = padding, halo_len,  halo_len, padding
		elif self.spatial_local_rank == 3:
			padding_left, padding_right, padding_top, padding_bottom = halo_len, padding, halo_len, padding
		else:#TBD: REPLACE 
			padding_left, padding_right, padding_top, padding_bottom = halo_len, padding, halo_len, padding
		self.pads = nn.ZeroPad2d((padding_left, padding_right, padding_top, padding_bottom))
...
class Pool(nn.Module):
	def __init__(self,
		...
		assert halo_len == padding[0], "halo_len should be equal to padding in pool layers "

		self.halo_len = halo_len
		self.padding = padding

		if(halo_len != 0):
			self.halo_len_layer = halo_exchange_layer(local_rank=local_rank,
		...
		if(operation == "MaxPool2d" ):
			...
			self.pool = getattr(torch.nn, 'MaxPool2d')(kernel_size = kernel_size,
											stride = stride,
											padding = 0,
											dilation = dilation,
			...
			# getattr(torch.nn, 'MaxPool2d')
			self.pool = getattr(torch.nn, 'AvgPool2d')(kernel_size = kernel_size,
											stride = stride,
											padding = 0,
											ceil_mode = ceil_mode,
