...
def get_shapes_spatial(shape_list, 
						slice_method, 
						spatial_size,
	...
	if(slice_method == "square"):
			...
			if(isinstance(output_shape, list)):
				...
				for shape_tuple in output_shape:
					if(temp_count<spatial_size):
						...
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times/2), int(shape_tuple[3]* image_size_times/2) )
						temp_shape.append(x)
					else:
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times), int(shape_tuple[3]* image_size_times) )
						temp_shape.append(x)
				...
				if(len(output_shape) == 2):
					...
					if(temp_count<spatial_size):
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times/2), int(output_shape[3]* image_size_times/2) )
						spatial_shapes_list.append(x)
					else:
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times), int(output_shape[3]* image_size_times) )
						spatial_shapes_list.append(x)
			temp_count += 1
		...
		for output_shape in shape_list:
			if(isinstance(output_shape, list)):
				...
				for shape_tuple in output_shape:
					if(temp_count<spatial_size):
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times/1), int(shape_tuple[3]* image_size_times/num_spatial_parts_list[temp_count]) )
						temp_shape.append(x)
					else:
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times), int(shape_tuple[3]* image_size_times) )
						temp_shape.append(x)
				...
				if(len(output_shape) == 2):
					...
					if(temp_count < spatial_size):
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times/1), int(output_shape[3]* image_size_times/num_spatial_parts_list[temp_count]) )
						spatial_shapes_list.append(x)
					else:
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times), int(output_shape[3]* image_size_times) )
						spatial_shapes_list.append(x)
		...
		for output_shape in shape_list:
			if(isinstance(output_shape, list)):
				...
				for shape_tuple in output_shape:
					if(temp_count < spatial_size):
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times/num_spatial_parts_list[temp_count]), int(shape_tuple[3]* image_size_times/1) )
						temp_shape.append(x)
					else:
						x = (int(shape_tuple[0]), shape_tuple[1], int(shape_tuple[2]* image_size_times), int(shape_tuple[3]* image_size_times) )
						temp_shape.append(x)
				...
				if(len(output_shape) == 2):
					...
					if(temp_count < spatial_size):
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times/num_spatial_parts_list[temp_count]), int(output_shape[3]* image_size_times/1) )
						spatial_shapes_list.append(x)
					else:
						x = (int(output_shape[0]), output_shape[1], int(output_shape[2]* image_size_times), int(output_shape[3]* image_size_times) )
						spatial_shapes_list.append(x)
...
def split_input(inputs, 
	...
	if(num_spatial_parts_list[0]==2):
		return split_input_2(	inputs, 
								image_size, 
								slice_method,
								local_rank)

	elif(num_spatial_parts_list[0] == 4):
		return split_input_4(	inputs, 
								image_size, 
								slice_method,
								local_rank)
...
class train_model_spatial(train_model):
	def __init__(self, model_gen, local_rank,batch_size,epochs, spatial_size=1, num_spatial_parts=4 ,criterion=None,optimizer=None,parts=1,ASYNC=True,GEMS_INVERSE=False, slice_method = "square",
						LOCAL_DP_LP=1,
						mpi_comm=None):
		...
		if (isinstance(num_spatial_parts, list)):
			...
			assert spatial_size == len(num_spatial_parts), "Spatial size is not equal to lenght of num_spatial_parts"
		else:
			self.local_rank = local_rank
		...
		if(local_rank < self.total_spatial_processes):
			...
			self.split_rank = math.floor((self.local_rank - self.total_spatial_processes ) / self.LOCAL_DP_LP) + spatial_size

		self.mp_size = mpi_comm.mp_size
		super(train_model_spatial, self).__init__(model_gen, local_rank,batch_size,epochs,criterion=criterion,optimizer=optimizer,parts=parts,ASYNC=ASYNC,GEMS_INVERSE=GEMS_INVERSE)
		...
		if(self.split_rank != 0 and self.local_rank < self.total_spatial_processes):
			recv_ranks_num = int (self.num_spatial_parts_list[self.split_rank-1] / self.num_spatial_parts_list[self.split_rank])
			if(recv_ranks_num > 1):
	...
	def initialize_recv_buffers_spatial_intermediate(self):
		...
		num_recvs = int(self.num_spatial_parts_list[self.split_rank-1] / self.num_spatial_parts_list[self.split_rank] )
				...
				if(self.split_rank!=0):
					...
					if(  isinstance(self.shape_list[self.split_rank-1],list)):
						for i in range(len(self.shape_list[self.split_rank-1])):
							one_input  = torch.zeros(self.shape_list[self.split_rank-1][i],requires_grad=True,device='cuda')
							input_x.append(one_input)
						input_x = tuple(input_x)
					else:

						input_x = torch.zeros(self.shape_list[self.split_rank-1],requires_grad=True,device='cuda')
		...
		if(self.split_rank!=self.split_size-1):
			if(  isinstance(self.shape_list[self.split_rank],list)):
				...
				for i in range(len(self.shape_list[self.split_rank])):
					temp_grad = torch.zeros(self.shape_list[self.split_rank][i],device='cuda')
					
					self.grad_overhead.append(temp_grad)
			else:
				self.grad_overhead = torch.zeros(self.shape_list[self.split_rank],device='cuda')
	...
	def initialize_recv_buffers_joint(self):
		...
		for part in range(self.parts):
			...
			for rank in ranks:
				...
				if(self.split_rank!=0):
					#multiple inputs
					print("Initialize recv buffer shape",self.shape_list[self.split_rank-1],list)
					if(  isinstance(self.shape_list[self.split_rank-1],list)):
						for i in range(len(self.shape_list[self.split_rank-1])):
							one_input  = torch.zeros(self.shape_list[self.split_rank-1][i],requires_grad=True,device='cuda')
							input_x.append(one_input)
	...
	def initialize_send_recv_ranks(self):
		...
		if( (self.split_rank == 0 and self.spatial_size == 1)  or 
			self.split_rank ==  self.spatial_size - 1 ):
			...
			send_ranks_num = int (self.num_spatial_parts_list[self.split_rank] / self.num_spatial_parts_list[self.split_rank+1])

			if(send_ranks_num == 1):
				self.increase_rank = self.num_spatial_parts
			else:

				recv_rank = sum(self.num_spatial_parts_list[:1]) + math.floor(self.spatial_local_rank / send_ranks_num )
		...
		if(self.GEMS_INVERSE==False):
			...
			print("initialize_send_recv_ranks Local rank(GEMS_INVERSE)",self.local_rank)
			self.to_send_forward = self.mp_size -1 - self.local_rank - self.increase_rank
			self.to_recv_forward = self.mp_size -1 - self.local_rank + self.decrease_rank
			self.to_send_backward = self.mp_size -1 - self.local_rank + self.decrease_rank
			self.to_recv_backward = self.mp_size -1 - self.local_rank - self.increase_rank

	def recv_input_spatial_async(self, part_number):

		num_recvs = int(self.num_spatial_parts_list[self.split_rank-1] / self.num_spatial_parts_list[self.split_rank] )

		first_local_rank_last_spatial = sum(self.num_spatial_parts_list[:self.split_rank-1])
		...
		for rank  in range(len(recv_ranks)):
			...
			if(self.MULTIPLE_INPUT==True):
				...
				for i in range(len(self.shape_list[self.split_rank-1])):
					...
					req_temp = dist.irecv(tensor=self.input_x_list[part_number][rank][i], src=recv_ranks[rank],tag=tag_forward)
					reqs.append(req_temp)
					tag_forward+=1
			else:
				req_temp = dist.irecv(tensor= self.input_x_list[part_number][rank], src = recv_ranks[rank], tag=tag_forward)
				reqs.append(req_temp)
		...
		for rank  in range(len(ranks)):
			...
			if(self.MULTIPLE_INPUT==True):
				...
				for i in range(len(self.shape_list[self.split_rank-1])):
					...
					req_temp = dist.irecv(tensor=self.input_x_list[part_number][rank][i], src=ranks[rank],tag=tag_forward)
					reqs.append(req_temp)
					tag_forward+=1
			else:
				req_temp = dist.irecv(tensor= self.input_x_list[part_number][rank], src = ranks[rank], tag=tag_forward)
				reqs.append(req_temp)
		...
		for partition in range(num_recvs):
			...
			if (self.MULTIPLE_INPUT):
				...
				for i in range(len(self.shape_list[self.split_rank-1])):
					...
					# to_send =input_x_list[:,:,partition_i*shape[2]:(partition_i+1)*shape[2],  partition_j*shape[3]:(partition_j+1)*shape[3]].grad.data.clone().detach()
					to_send = input_x_list[partition][i].grad.data.clone().detach()
					req = dist.isend(tensor=to_send, dst=recv_ranks[partition], tag = tag_forward)
					tag_forward+=1
					reqs.append(req)
	...
	def send_input_spatial_MP_joint_LP_DP(self,y):
		...
		assert parts_size%self.LOCAL_DP_LP==0, "Part size (batch size/parts) should be divided by Local DP LP "
		...
		for rank  in range(prev_num_spatial_parts):
			...
			if(self.MULTIPLE_INPUT==True):
				...
				for i in range(len(self.shape_list[self.split_rank-1])):
					...
					if(self.GEMS_INVERSE):
						src = self.mp_size - 1 - src 
					dist.scatter(tensor = self.input_x_list[part_number][rank][i], 
						scatter_list= None,
						src = src,
				...
				if(self.GEMS_INVERSE):
					src = self.mp_size - 1 - src 
				dist.scatter(tensor = self.input_x_list[part_number][rank], 
						scatter_list= None,
						src = src,
						group = self.LP_DP_Group_List[rank])
				# reqs.append(req_temp)
	...
	def merge_inputs_joint(self,part_number):
		...
		if(self.MULTIPLE_INPUT):
			for i in range(len(self.shape_list[self.split_rank-1])):
				...
				for partition_i in range(int(math.sqrt(self.num_spatial_parts))):
					for partition_j in range(int(math.sqrt(self.num_spatial_parts))):
						self.joint_inputs[part_number][i][:,:, partition_i*shape[2]:(partition_i+1)*shape[2],  partition_j*shape[3]:(partition_j+1)*shape[3] ].set_(self.input_x_list[part_number][partition_i * int(math.sqrt(self.num_spatial_parts)) + partition_j][i])
			...
			for partition_i in range(int(math.sqrt(self.num_spatial_parts))):
				for partition_j in range(int(math.sqrt(self.num_spatial_parts))):
					self.joint_inputs[part_number][:,:, partition_i*shape[2]:(partition_i+1)*shape[2],  partition_j*shape[3]:(partition_j+1)*shape[3] ].set_( self.input_x_list[part_number][partition_i * int(math.sqrt(self.num_spatial_parts)) + partition_j])
			...
			if(self.MULTIPLE_INPUT):
				...
				for i in range(len(self.shape_list[self.split_rank-1])):
					...
					for partition_i in range(int(math.sqrt(self.num_spatial_parts))):
						temp_col = []
						for partition_j in range(int(math.sqrt(self.num_spatial_parts))):
							temp_col.append(self.input_x_list[part_number][partition_i * int(math.sqrt(self.num_spatial_parts)) + partition_j][i])
				...
				for partition_i in range(int(math.sqrt(self.num_spatial_parts))):
					temp_col = []
					for partition_j in range(int(math.sqrt(self.num_spatial_parts))):
						temp_col.append(self.input_x_list[part_number][partition_i * int(math.sqrt(self.num_spatial_parts)) + partition_j])
	...
	def merge_inputs_intermediate_spatial(self,part_number):
		...
		assert self.slice_method != "square", "Currently different num_spatial_parts are not supported for square slice method"
	...
	def forward_pass(self,data_x,data_y,part_number=0):
		...
		if(isinstance(self.models, torch.nn.parallel.distributed.DistributedDataParallel) and 
			part_number != self.parts-1):
		...
		if(self.split_rank!=self.split_size-1):
			if(self.ENABLE_ASYNC==True):
				if(self.split_rank == self.spatial_size-1 and
					self.ENABLE_LOCAL_DP_LP):
			...
			if(self.split_rank == self.spatial_size):
				...
				if(self.MULTIPLE_INPUT):
					for i in range(self.num_spatial_parts):
						self.input_x_list[part_number][i] = list(self.input_x_list[part_number][i])
						for j in range(len(self.shape_list[self.split_rank-1])):

							self.input_x_list[part_number][i][j] = self.input_x_list[part_number][i][j].detach().requires_grad_()
						self.input_x_list[part_number][i] = tuple(self.input_x_list[part_number][i])

				else:
					for i in range(self.num_spatial_parts):
						self.input_x_list[part_number][i] = self.input_x_list[part_number][i].detach().requires_grad_()
				...
				if(self.MULTIPLE_INPUT):
					...
					for j in range(len(self.shape_list[self.split_rank-1])):
						self.input_x_list[part_number][j] = self.input_x_list[part_number][j].detach().requires_grad_()
					self.input_x_list[part_number]= tuple(self.input_x_list[part_number])
				else:


					self.input_x_list[part_number] = self.input_x_list[part_number].detach().requires_grad_()
