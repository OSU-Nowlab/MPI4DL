...
class train_spatial_model_master:
	...
	def __init__(self, 	model_gen1, 
		...
		self.flat_params_model1 = torch.zeros([self.model1_size],requires_grad=True,device='cuda')	

		self.flat_params_model2 = torch.zeros([self.model2_size],requires_grad=True,device='cuda')

		self.flat_grads_model1 = torch.zeros([self.model1_size],requires_grad=False,device='cuda')	

		self.flat_grads_model2 = torch.zeros([self.model2_size],requires_grad=False,device='cuda')		
		...
		self.train_model1 = train_model_spatial(model_gen1, mpi_comm_first.local_rank,batch_size,
												epochs=1, 
		...
		self.train_model2 = train_model_spatial(model_gen2, mpi_comm_second.local_rank,batch_size,
												epochs=1, 
												spatial_size=spatial_size, 
												num_spatial_parts=num_spatial_parts,
												criterion=criterion,
		...
		for param in model_gen.models.parameters():
			if(param is not None):
				if flat_params is None:
					...
					flat_params = torch.cat((flat_params,param.data.clone().detach().view(-1)))
	...
	def send_recv_grads(self, odd_iteration=False):
		...
		if(odd_iteration):
			...
			flat_grads_recv = torch.zeros([self.model1_size],requires_grad=False,device='cuda')		
		else:

			flat_grads_recv = torch.zeros([self.model2_size],requires_grad=False,device='cuda')	
	...
	def run_step_allreduce(self, inputs, labels, odd_iteration):
		...
		if(tm1.local_rank==self.mp_size-1):
			...
			req1 = self.recv_params_model(recv_rank=recv_rank,odd_iteration=odd_iteration)
			req1.wait()
		...
		for i in range(self.parts):
			...
			temp_y,temp_correct = tm1.forward_pass(data_x[start:end],data_y[start:end],part_number=i)
		...
		if(tm1.local_rank == self.mp_size-1):
			...
			req1 = self.send_params_model(send_rank=send_rank,odd_iteration=odd_iteration)
			req1.wait()
		...
		for i in range(self.parts):
			...
			temp_y,temp_correct = tm2.forward_pass(data_x[start:end],data_y[start:end],part_number=i)
			y_list.append(temp_y)
		...
		if(tm2.local_rank == self.mp_size-1):
			...
			send_rank = self.mp_size - 1 - self.local_rank
			req1 = self.send_grads_model(send_rank=send_rank,odd_iteration=odd_iteration)
			req1.wait()
	...
	def run_step(self,inputs,labels):
		...
		temp_loss,temp_correct =  self.train_model1.run_step(inputs[:self.batch_size], labels[:self.batch_size])
		loss += temp_loss
		...
		temp_loss ,temp_correct= self.train_model2.run_step(inputs[self.batch_size:2*self.batch_size], labels[self.batch_size:2*self.batch_size])
		...
		for times in range(self.replications-1):
			index = (2 * times) + 2
			temp_loss ,temp_correct= self.train_model1.run_step(inputs[index*self.batch_size:(index+1)*self.batch_size], labels[index*self.batch_size:(index+1)*self.batch_size])
			loss += temp_loss
			correct += temp_correct

			temp_loss ,temp_correct= self.train_model2.run_step(inputs[(index+1)*self.batch_size:(index+2)*self.batch_size], labels[(index+1)*self.batch_size:(index+2)*self.batch_size])
